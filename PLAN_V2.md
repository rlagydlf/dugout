...
### Phase 3 — 효과·애니메이션·3D 강화 (60분)

**산출물**: 화려·인터랙티브 유지하되 의도적이고 일관된 모션 시스템

- [ ] **회전 야구공 ambient 유지** + Y축 3D Tilt 추가 (스크롤 시 약간 기울임)
- [ ] **다이아몬드 그리드 패턴 정제** — 다양한 시각 표현으로 화면별 차별화
- [ ] **CRT 스캔라인 유지** + 라이브 깜빡 효과 추가
- [ ] **펄스 글로우 강화** — 팀 컬러 다중 펄스 (메인 1 + 액센트 2)
- [ ] **그라데이션 카드** — 더 정교한 다중 stops (3~5 color stops + 미세한 noise overlay)
- [ ] **3D Card Tilt** — 카드 tap 시 perspective transform (X/Y rotation 5도)
- [ ] **Parallax 스크롤** — 홈 hero 마스코트 스크롤 시 시차 이동
- [ ] **Particle 효과** — 출정 / 예측 적중 / 리워드 교환 시 파티클 시스템 (별/하트/번개)
- [ ] **Shader 효과** — 매치업 카드 hover 시 glow shader 적용
- [ ] **Motion Curves** — Curves.easeOutBack / elasticOut / decelerate 표준화

**검증**: 시뮬레이터에서 화려한 인터랙션 + 일관된 모션 체크
